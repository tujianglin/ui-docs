...
export default function useLocale(
  ...
  showProps: ReactiveComputedReturn<
    Pick<SharedTimeProps, 'showHour' | 'showMinute' | 'showSecond' | 'showMillisecond' | 'use12Hours'>
  >,
) {
  const { showHour, showMinute, showSecond, showMillisecond, use12Hours } = $(showProps);
  return computed<Locale>(() => fillLocale(locale.value, showHour, showMinute, showSecond, showMillisecond, use12Hours));
}
