...
export function getRowFormat(picker: InternalMode, locale: Locale, format?: SharedPickerProps['format']) {
  if (format) {
    return format;
  }
...
export function getFromDate<DateType>(calendarValues: DateType[], activeIndexList: number[], activeIndex?: number) {
  const mergedActiveIndex = activeIndex !== undefined ? activeIndex : activeIndexList[activeIndexList.length - 1];
