import { getFocusNodeList } from '@vc-com/util/lib/Dom/focus';
...
function findContainerUL(element: HTMLElement): HTMLUListElement {
  let value: HTMLElement = element;
  while (value) {
    if (value.getAttribute('data-menu-list')) {
      return value as HTMLUListElement;
    }
...
function getFocusElement(activeElement: HTMLElement, elements: Set<HTMLElement>): HTMLElement {
  let value = activeElement || document.activeElement;

  while (value) {
    if (elements.has(value as any)) {
...
function getNextFocusElement(
  ...
  // List value level menu item elements
...
export function useAccessibility(
  ...
  return (e: KeyboardEvent) => {
    ...
    if ([...ArrowKeys, ENTER, ESC, HOME, END].includes(which)) {
      ...
      // First we should find value focused MenuItem/SubMenu element
