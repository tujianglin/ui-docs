...
export function getVisibleArea(initArea: VisibleArea, scrollerList?: HTMLElement[]) {
  ...
  (scrollerList || []).forEach((ele) => {
    ...
    const eleRight =
      eleLeft + eleRect.width + 2 * clipMarginWidth - scaledBorderLeftWidth - scaledBorderRightWidth - eleScrollWidth;

    const eleBottom =
      eleTop + eleRect.height + 2 * clipMarginHeight - scaledBorderTopWidth - scaledBorderBottomWidth - eleScrollHeight;
