import { isDOM } from '@vc-com/util/lib/Dom/findDOMNode';
import isVisible from '@vc-com/util/lib/Dom/isVisible';
import { computed, nextTick, reactive, shallowRef, toRefs, watch, type Ref } from 'vue';
...
function getNumberOffset(rect: { width: number; height: number }, offset?: OffsetType[]) {
...
export default function useAlign(
  ...
  const onAlign = () => {
    if (popupEle.value && target.value && open.value && !mobile.value) {
      ...
      function getIntersectionVisibleArea(offsetX: number, offsetY: number, area = visibleArea) {
  ...
  const { ready, offsetX, offsetY, offsetR, offsetB, arrowX, arrowY, scaleX, scaleY, align } = toRefs(offsetInfo);

  return [ready, offsetX, offsetY, offsetR, offsetB, arrowX, arrowY, scaleX, scaleY, align, triggerAlign];
}
