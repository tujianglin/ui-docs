 ...
 */
export function triggerFocus(element?: HTMLElement, option?: InputFocusOptions) {
  if (!element) return;
...
function syncFocus() {
  ...
  if (lastElement && !hasFocus(lastElement)) {
    ...
    const matchElement = focusableList.includes(lastFocusElement as HTMLElement) ? lastFocusElement : focusableList[0];
