...
import type { CSSProperties } from '../setStyle';
...
export default class ScrollLocker {
  ...
  lock = () => {
    ...
    if (
      (container === document.body && window.innerWidth - document.documentElement.clientWidth > 0) ||
  ...
  unLock = () => {
    ...
    if (!findLock || locks.some(({ options }) => options?.container === findLock.options?.container)) {
      return;
    }
